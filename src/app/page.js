...
import { collection, getDocs, query, doc, getDoc, setDoc, deleteDoc, updateDoc } from 'firebase/firestore';
        ...
        <Stack width={"800px"} height={"300px"} spacing={2} overflow={"auto"}>
          {
            inventory.map(({id, quantity}) => (
              <Box key={id} width={"100%"} minHeight={"150px"} display={"flex"} alignItems={"center"} justifyContent={"space-between"} bgcolor={"#f0f0f0"} padding={5}>
                <Typography variant='h3' color={"#333"} textAlign={"center"}>{id.charAt(0).toUpperCase() + id.slice(1)}</Typography>
